...
module cmsdk_iop_gpio
 #(// Parameter to define valid bit pattern for Alternate functions
   ...
   //
   // By default every bit can have alternate function 默认情况下每个位都可以具有备用功能
   parameter  ALTERNATE_FUNC_MASK = 16'hFFFF,

   // Default alternate function settings 默认备用功能设置
...
// easy to customise the width.    gpio宽度默认16位
localparam  PortWidth                      = 16;
// Local parameter for IDs, IO PORT GPIO has part number of 820 ID的本地参数，IO端口GPIO的部件号为820
...
//          - Optional revision and modification number (e.g. rXpY)
  ...
  case (IOADDR[11:10])
    ...
    2'b11: begin
           if (IOADDR[9:6]==4'hF) // Peripheral IDs and Component IDs. 外围设备ID和组件ID 
